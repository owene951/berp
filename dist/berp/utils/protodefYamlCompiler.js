...
exports.ProtoDefYAMLParse = exports.ProtoDefYAMLCompile = void 0;
...
const logger = new console_1.Logger('Protodef YAML Compiler', 'yellow');
const log = (...idk) => { };
...
function toYAML(input, followImports = true, document = false) {
    ...
    if (typeof input === 'string') {
        files.main = fs_1.default.readFileSync(input, 'utf8');
    }
    else {
        Object.assign(files, input);
    }
    const imported = [];
    function checkIfJson(key, val) {
        if (key.includes('"') || val.includes('[[') || val.includes('[{') || val.includes('{') || val.includes('}'))
            return true;
    ...
    function pars() {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (!key)
                continue;
            if (key.startsWith('!')) {
                if (key.startsWith('!StartDocs'))
                    startedDocumenting = true;
                if (key.startsWith('!EndDocs'))
                    startedDocumenting = false;
                if (key == '!import' && !imported.includes(val) && followImports) {
                    ...
                    if (!imp && typeof input === 'string') {
                        ...
                        imp = fs_1.default.readFileSync(path_1.default.dirname(input) + '/' + val, 'utf-8');
                    }
                    else if (!imp) {
                ...
                if (key == '!import' || key.includes('Docs'))
                    nkey += ',' + i;
                nkey += "'";
            ...
            if (isParent) {
                ...
                if (val.includes('[]')) {
                    ...
                    if (type)
                        throw Error('Array type cannot be both container and ' + type + ' at ' + val);
    ...
    while (pars()) {
        logger.info('Importing', imported[imported.length - 1], "...");
    }
    log(lines);
...
function parseYAML(lines) {
    try {
        let ret;
        js_yaml_1.default.loadAll(lines.join('\n'), d => ret = d);
        return ret;
    }
    catch (e) {
        ...
        if (typeof obj === 'string') {
            ...
            if (len == 1 && (first.startsWith('%array') || first.startsWith('%switch'))) { // remove container nested array
                if (name) {
                    ...
                    if (!a.type[0].name || a.type[0].name.startsWith('__'))
                        a.type = a.type[0].type;
                    else
                        a.type = ['container', [a.type[0]]];
                }
                else {
                    const a = {
                        countType,
            ...
            if (typeof val === 'object') {
                if (key.startsWith('%')) {
                    ...
                    if (key.startsWith('%map')) {
                        ...
                        if (type && val && typeof val === 'object')
                            throw Error('Array has a type and body: ' + name);
...
exports.ProtoDefYAMLParse = getIntermediate;
function compile(input, output) {
    const ret = formFinal(transform(parseYAML(toYAML(input))));
    if (typeof output === 'string')
        fs_1.default.writeFileSync(output, JSON.stringify(ret, null, 2));
    return ret;
}
exports.ProtoDefYAMLCompile = compile;
